...
set<int> Check(QNode* Qroot)
{
	if(Qroot->data=="+")
	{
		set<int> tmpc, tmpa, tmpb;
		tmpa =  Check(Qroot->left);
		tmpb = Check(Qroot->right);
	...
	else
	{
		...
		else if(Qroot->data[0]=='*') 
		{
			...
			copy.erase(0,1);
			copy.pop_back();
			return findTrie_suff(copy);
		}
		else return findTrie_pre(Qroot->data);
	}
}
